...
class Jarvis:
    ...
    def speak(self, text):
        ...
        self.speaking = True
        if self.gui:
            self.gui.switch_gif("images/v2.gif")  # Set speaking GIF
        self.engine.say(text)
        self.engine.runAndWait()
        self.speaking = False
        if self.gui:
            self.gui.switch_gif("images/v1.gif") 
    ...
    def respond(self, user_input):
        ...
        if (("current" in user_input_lower and "weather" in user_input_lower) or 
         ...
         # ----- NEWS FEATURE -----
        if "show todays news" in user_input_lower or "show today's news" in user_input_lower or "show news" in user_input_lower or "news" in user_input_lower or "today's news" in user_input_lower or "todays news" in user_input_lower or "live news" in user_input_lower or "latest news" in user_input_lower:
        ...
        pic_match = re.search(r'(?:display|show|view|get) (?:image|picture|photo|pic)(?: of)? (.+)', user_input, re.IGNORECASE)
        if pic_match:
        ...
        if "next" in user_input_lower or "next reel" in user_input_lower or "scroll" in user_input_lower or "scroll down" in user_input_lower:
        ...
        if "type message" in user_input_lower:
        ...
        if ("text editor" in user_input_lower or "text file" in user_input_lower or
            ...
            if self.handle_text_editor_commands(user_input):
                return
        elif "close" in user_input_lower and ("notepad" in user_input_lower or "text editor" in user_input_lower or "tab" in user_input_lower or "window" in user_input_lower ):
        ...
        if any(phrase in user_input_lower for phrase in [
                "what's the time", "tell me the time", "current time", "what is the time",
                "what's the date", "tell me the date", "current date", "what is the date",
                "what's the day", "tell me the day", "current day", "what is the day"]):
    ...
    def handle_youtube_commands(self, user_input):
        ...
        if "forward" in user_input or "skip 10 seconds" in user_input:
                pyautogui.press("l")
    ...
    def handle_code_requests(self, user_input):
        ...
        update_match = (re.search(r"update line (\d+) to (.+)", user_input, re.IGNORECASE)) or (re.search(r"go to line (\d+)  update and write  (.+)", user_input, re.IGNORECASE))
...
class FloatingGUI(tk.Tk):
    ...
    def switch_gif(self, gif_path):
        ...
        self.after(300, self.animate_gif) 
        self.load_and_animate_gif(self.current_gif_path, (500, 400))
